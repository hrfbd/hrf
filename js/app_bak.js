...
function applyDynamicImages() {
  ...
  if (logo && db.data.settings?.media?.siteLogo) {
    logo.src = db.data.settings.media.siteLogo;
  }
  
  const hero = document.getElementById('dynamic-hero-img');
  if (hero && db.data.settings?.media?.heroImage) {
    hero.src = db.data.settings.media.heroImage;
  }
  
  if (db.data.settings?.media?.foodActivityImage) {
    document.querySelectorAll('.dynamic-food-img').forEach(img => img.src = db.data.settings.media.foodActivityImage);
  }
  
  if (db.data.settings?.media?.eduActivityImage) {
    document.querySelectorAll('.dynamic-edu-img').forEach(img => img.src = db.data.settings.media.eduActivityImage);
  }
  
  if (db.data.settings?.media?.oldAgeHomeImage) {
    document.querySelectorAll('.dynamic-oldage-img').forEach(img => img.src = db.data.settings.media.oldAgeHomeImage);
  }
}
    ...
    contentHtml = `
      ...
      <div style="background:rgba(212, 175, 55, 0.1); border-left:4px solid var(--accent-gold); padding:1rem; margin-bottom:1.5rem; border-radius:0 var(--radius-md) var(--radius-md) 0;">
        ...
        <p style="font-size:0.9rem; color:var(--text-muted); line-height:1.4;">
          <strong>${nameBn}</strong> বর্তমানে এক মুঠো খাবার মানবিক ফাউন্ডেশনের <strong>${titleBn}</strong> হিসেবে গুরুত্বপূর্ণ দায়িত্ব পালন করছেন।
        </p>
      </div>
    ...
    contentHtml = `
      ...
      <div style="background:rgba(212, 175, 55, 0.1); border-left:4px solid var(--accent-gold); padding:1.25rem; margin-bottom:1.5rem; border-radius:0 var(--radius-md) var(--radius-md) 0;">
        ...
        <p style="font-size:0.95rem; color:var(--text-muted); line-height:1.5;">
          <strong>${nameBn}</strong> বর্তমানে এক মুঠো খাবার মানবিক ফাউন্ডেশনের <strong>${titleBn}</strong> হিসেবে অত্যন্ত নিষ্ঠার সাথে গুরুত্বপূর্ণ দায়িত্ব পালন করছেন।
        </p>
      </div>
  ...
  container.innerHTML = notices.map(n => `
    <div style="background:#fff; border-radius:var(--radius-lg); padding:1.75rem; box-shadow:var(--shadow-sm); border:1px solid var(--border-color); border-top:4px solid var(--primary-mid); display:flex; flex-direction:column; justify-space-between;">
      ...
      <div style="margin-top:1.25rem; padding-top:1rem; border-top:1px dashed var(--border-color); display:flex; align-items:center; gap:0.5rem; font-size:0.8rem; color:var(--primary-deep); font-weight:600;">
        <i class="fas fa-shield-alt"></i> এক মুঠো খাবার মানবিক ফাউন্ডেশন অফিশিয়াল ঘোষণা
      </div>
    </div>
    </div>
  `).join('');
      ...
      <div class="form-group">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem;">
          <label style="border:1px solid var(--border-color); padding:0.6rem; border-radius:var(--radius-md); display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
            <input type="radio" name="pay_method" value="bKash" checked> <span>bKash (বিকাশ)</span>
          </label>
          <label style="border:1px solid var(--border-color); padding:0.6rem; border-radius:var(--radius-md); display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
            <input type="radio" name="pay_method" value="Nagad"> <span>Nagad (নগদ)</span>
          </label>
          <label style="border:1px solid var(--border-color); padding:0.6rem; border-radius:var(--radius-md); display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
            <input type="radio" name="pay_method" value="Bank Transfer"> <span>Bank Transfer</span>
          </label>
          <label style="border:1px solid var(--border-color); padding:0.6rem; border-radius:var(--radius-md); display:flex; align-items:center; gap:0.5rem; cursor:pointer;">
...
function showMemberProfile(memberId) {
  ...
  const html = `
    <div style="text-align:center; margin-bottom:1.5rem;">
      ${avatarHtml}
      <h3 style="font-size:1.4rem; color:var(--primary-deep); margin-top:1rem; margin-bottom:0.25rem;">${member.name}</h3>
      <span class="badge badge-verified">${member.typeLabelBn || member.type}</span>
    </div>
